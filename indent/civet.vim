...
if exists('g:civet_indent_keep_current')
...
let s:SYNTAX_STRING = 'civet\%(String\|AssignString\|Regex\)'

" Syntax names for comments
let s:SYNTAX_COMMENT = 'civet\%(Comment\)'
...
function! GetCivetIndent(curlnum)
  ...
  if has_key(s:BRACKET_PAIRS, curline[0])
    ...
    let matchlnum = s:SearchPair(a:curlnum, a:curlnum - s:MAX_LOOKBACK,"s:IsCommentOrString(line('.'), col('.'))",s:BRACKET_PAIRS[curline[0]], curline[0])
  ...
  let matchlnum = s:SearchPair(a:curlnum, a:curlnum - s:MAX_LOOKBACK,"s:IsCommentOrString(line('.'), col('.'))",'\[\|(\|{', '\]\|)\|}')
